...
if question:
    ...
    with st.chat_message("assistant"):
            ...
            try:
                ...
                if "429" in str(e) or "quota" in str(e).lower() or "RESOURCE_EXHAUSTED" in str(e):
                    st.error(
                        ...
                        "Please try again after the quota resets or use a billing-enabled Gemini API project."
                    )
                    st.stop()
                raise
        ...
        if result.confidence_label:
            st.caption(f"Retrieval signal: {result.confidence_label} · Response time: {elapsed_ms:.0f} ms")
